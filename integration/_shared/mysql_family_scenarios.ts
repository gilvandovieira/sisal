...
export function mysqlFamilyScenarios(): IntegrationScenario[] {
  ...
  mysqlTest(
    "mysql: numeric + bigint decode as precision-safe strings",
    async (db) => {
      ...
      await dropTables(db, ["it_numbers"]);
      await applyTables(db, [nums]);
      // `big` sits above 2^53: the driver hands DECIMAL/BIGINT back as strings,
      // so precision survives on both MySQL and MariaDB.
      ...
      assertEquals(typeof row.amount, shape.numeric); // "string" for the family
      assertEquals(typeof row.big, shape.bigint); // "string" for the family
  ...
  mysqlTest(
    "mysql: advisory lock — coarse run mutual exclusion",
    async (db) => {
      await dropTables(db, ["it_run_locks"]);
  ...
  mysqlTest(
    "mysql: checkpoint — atomic load+advance + idempotent resume",
    async (db) => {
      await dropTables(db, ["it_rollup", "sisal_etl_checkpoints"]);
      await db.execute(
        raw("create table it_rollup (k int primary key, v int not null)"),
      );
  ...
  mysqlTest(
    "mysql: checkpoint contract — round-trip, multi-job, persistence",
    async (db) => {
      await dropTables(db, ["sisal_etl_checkpoints"]);
      ...
      // Exact TEXT fidelity: byte-identical read-back on both MySQL and MariaDB
      // (a varchar column, so no DATETIME truncation of the ISO string).
  ...
  mysqlTest(
    "mysql: checkpoint retention — prune horizon + replay refusal",
    async (db) => {
      await dropTables(db, ["it_src", "sisal_etl_checkpoints"]);
      await db.execute(
        raw(
          "create table it_src (id int primary key, ts varchar(32) not null)",
        ),
      );
  ...
  mysqlTest("mysql: write outcome — inserted vs conflicted", async (db) => {
    await dropTables(db, ["it_wo"]);
    await db.execute(raw("create table it_wo (k int primary key)"));
    const t = defineTable("it_wo", { k: columns.integer().primaryKey() });

    // The MySQL family has no RETURNING here — tryInsert reads the affected-row
    // count of the no-op `ON DUPLICATE KEY UPDATE` (1 on insert, 0 on conflict).
  ...
  mysqlTest("mysql: read cte — WITH on SELECT", async (db) => {
    await dropTables(db, ["it_cte"]);
    await db.execute(
      raw("create table it_cte (id int primary key, n int not null)"),
    );
  ...
  mysqlTest("mysql: recursive cte — WITH RECURSIVE walk", async (db) => {
    // WITH RECURSIVE needs MySQL 8.0+ / MariaDB 10.2+ (both integration targets
    // clear that floor; below it, MySQL 5.7 has no recursive CTE).
    await dropTables(db, ["it_tree"]);
    await db.execute(
      raw("create table it_tree (id int primary key, parent_id int)"),
    );
