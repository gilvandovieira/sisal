...
export function postgresFamilyScenarios(): IntegrationScenario[] {
  const scenarios: IntegrationScenario[] = [];
  const pgTest = scenarioCollector(scenarios);
  ...
  pgTest("pg: filter operators", async (db) => {
    ...
    assertEquals(
      (await db.select().from(users).where(gte(users.columns.age, 17))
        .execute())
        .length,
      3,
    );
    ...
    assertEquals(
      (await db.select().from(users).where(lte(users.columns.age, 30))
        .execute())
        .length,
      2,
    );
    ...
    assertEquals(
      (await db.select().from(users).where(
        notBetween(users.columns.age, 18, 40),
      )
        .execute()).length,
      2,
    );
  ...
  pgTest(
    "pg: upsert (onConflictDoNothing / onConflictDoUpdate)",
    async (db) => {
  ...
  pgTest(
    "pg: ETL rollup (insert-from-select + FILTER + dateTrunc + upsert)",
    async (db) => {
      await db.execute(
        raw("drop table if exists it_roll_events, it_roll_hourly cascade"),
      );
      ...
      for (
        const stmt of generatePostgresUpStatements(
          createSchemaSnapshot({ dialect: "postgres", tables: [ev, hourly] }),
        ).statements
      ) await db.execute(stmt);
      ...
      // FILTER aggregates over dateTrunc hour buckets, upserted ON CONFLICT.
      const e = ev.columns;
  ...
  pgTest("pg: schema objects (functions/triggers/views)", async (db) => {
    ...
    await db.insert(it_so).values({ id: 1, label: "x", stamped: null })
      .execute();
  ...
  pgTest("pg: teardown", async (db) => {
    ...
    await db.execute(
      raw(
        "drop table if exists it_all_types, it_posts, it_users, it_orgs, it_bin, it_widget, it_history, it_accounts, it_legacy, it_feed, it_temporal_values, it_expr, it_batch, it_rich_idx, it_floats, it_counters, it_dmcte, it_so, it_agg, it_rawmap, it_dm, it_dmbin, it_mj, it_mj_arch, it_roll_events, it_roll_hourly cascade",
      ),
    );
    await db.execute(
      raw(
