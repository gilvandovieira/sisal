...
function buildLlmsTxt(
  ...
  const lines: string[] = [
    ...
    "",
    `- [Full LLM Context](${SITE_BASE_URL}/llms-full.txt): Generated combined context from Sisal docs, package manifests, and exported API docs.`,
  ];
...
async function buildLlmsFullTxt(
  ...
  const lines: string[] = [
    "# Sisal Full LLM Context",
    "",
    "> Generated by `deno task docs:llms` from Sisal source documentation, package manifests, and `deno doc --json` output for public exports.",
    "",
    "## Compact Index",
    "",
  ...
  lines.push("## Source Documentation", "");

  for (const doc of docs) {
    lines.push(`### ${doc.path}`, "");
    lines.push(`URL: ${doc.url}`, "");
    lines.push("```md");
    lines.push(stripTrailingNewline(doc.content));
    lines.push("```", "");
  }
