...
/**
 * Opens a SQLite database with the runtime-native driver, imported lazily:
 * Deno uses the FFI-backed `@db/sqlite`, Node the built-in `node:sqlite` (see
 * {@link hasDenoFfi}). Importing `@sisal/migrate/sqlite` — and running its
 * fake-backed tests — needs no permissions; only a real Deno open needs
