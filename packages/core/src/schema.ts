...
export interface SisalColumnSnapshot {
   ...
   * STORED|VIRTUAL`). The `sql` text is emitted into DDL verbatim.
   *
   * **Trusted input.** Set it only from developer-authored schema code — see
   * `docs/security.md` (SEC-006). PostgreSQL supports only `stored: true`;
   * the SQLite and MySQL families also accept `VIRTUAL`.
   */
