...
Deno.test("analytics: duplicate keys fail at declaration time", () => {
  const duplicateMetric = assertThrows(
    () =>
      from(postHourlyStats)
        .dimensions({ bucket: bucket("hour", s.bucket) })
        .metrics({ bucket: sum(s.votes) }),
    OrmError,
  );
  assertEquals(duplicateMetric.code, "ORM_INVALID_QUERY");
  assertStringIncludes(duplicateMetric.message, 'key "bucket"');
  assertStringIncludes(duplicateMetric.message, "already declared");

  const duplicateWindow = assertThrows(
    () =>
      from(postHourlyStats)
        .dimensions({ bucket: bucket("hour", s.bucket) })
        .metrics({ votes: sum(s.votes) })
        .windows({
          votes: movingAvg("votes", { orderBy: ["bucket"], rows: 2 }),
        }),
    OrmError,
  );
  assertEquals(duplicateWindow.code, "ORM_INVALID_QUERY");
  assertStringIncludes(duplicateWindow.message, 'key "votes"');
  assertStringIncludes(duplicateWindow.message, "already declared");
});
