...
export interface SisalCliGeneratedStatements {
  /** Whether this sisal cli generated statements includes destructive changes. */
...
export interface SisalCliMigrator {
  /** Applies pending migrations for this sisal cli migrator. */
  migrate(
    options: MigrationRunOptions & {
      readonly migrations: readonly Migration[];
    },
  ): Promise<MigrationResult>;
  /** Builds a migration plan for this sisal cli migrator. */

  plan(options: {
    readonly migrations: readonly Migration[];
  }): Promise<MigrationPlan>;
  /** Lists applied migrations for this sisal cli migrator. */

  applied?(): Promise<unknown>;
  /** Closes resources held by this sisal cli migrator. */
...
export interface SisalCliOptions {
  /** fs for this sisal cli options. */
  readonly config?: MigrateConfig;
  /** adapters for this sisal cli options. */
  readonly fs?: MigrationFileSystem;
  /** cwd for this sisal cli options. */
  readonly adapters?: Partial<Record<CliDialect, SisalCliAdapter>>;
  /** Logging options used by this sisal cli options. */
  readonly cwd?: string;
  /** stdout for this sisal cli options. */
  readonly logging?: SisalLogSettings;
  /** stderr for this sisal cli options. */
...
const DEFAULT_ADAPTER_VERSION = "^0.11.0";
const DEFAULT_MYSQL_ADAPTER_VERSION = "^0.11.0";
...
function createCliLogger(out: CliOutput, err: CliOutput): Logger {
  return {
    log(event) {
      const sink = event.level === "error" || event.level === "warn"
        ? err
        : out;
      const record = event.record;
      const level = logRecordString(record, "level") ?? event.level;
      const category = logRecordString(record, "category") ?? event.category;
      ...
      sink(`[${level}] ${category}: ${event.message}${suffix}`);
    },
  };
}
