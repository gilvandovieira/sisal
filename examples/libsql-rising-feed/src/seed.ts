 ...
 *
 * Same dataset and goals as the Neon sibling (it proves the same six product
 * behaviors), but here activity is recorded through the TypeScript
 * `recordPostActivity` (a transaction), not a SQL function, and timestamps are
 * ISO-8601 strings. Determinism: every timestamp derives from a fixed
 * `DEMO_NOW`, never the wall clock, and the rising score is recomputed at that
 * same `DEMO_NOW`.
 *
 * @module
 */
