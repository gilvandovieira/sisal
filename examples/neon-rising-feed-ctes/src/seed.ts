 ...
 *   7. old activity falls out of the window when DEMO_NOW advances.
 *
 ...
 *
 * Activity is recorded through the live `recordPostActivity` CTE — one statement
 * per event — so seeding exercises the same recorder the demo and tests use.
 *
 * @module
 */
...
import { type ActivityKind, recomputeAllRisingScores } from "./rising.ts";
...
const POST_SPECS: readonly PostSpec[] = [
  ...
  {
    key: "fresh-burst",
    title: "Show: a tiny moving-average ranker in one CTE",
  ...
  {
    key: "comment-storm",
    title: "Hot take: data-modifying CTEs are underrated",
    body: "Spicy thread.",
    hoursAgo: 1.5,
    activity: [
...
export async function seed(
  ...
  const dateAt = (msAgo: number) => new Date(nowMs - msAgo);
  ...
  const postRows = POST_SPECS.map((spec) => {
    ...
    return {
      ...
      created_at: dateAt(spec.hoursAgo * HOUR_MS),
    };
  });
  await db.insert(posts).values(postRows).execute();

  // Record activity event-by-event through the recording CTE. Stable actors
  // share one UUID per (post, actor key); "distinct" gets a fresh UUID.
  for (let i = 0; i < POST_SPECS.length; i += 1) {
    ...
    for (const event of spec.activity) {
      const at = dateAt(event.minutesAgo * MINUTE_MS);
