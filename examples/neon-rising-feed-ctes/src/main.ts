/**
 * Demo: prove the CTE-driven rising feed end to end against Neon/Postgres.
 *
 ...
 *   2. Recompute all rising scores (one bulk CTE) and print /new + /rising.
 *   3. Record fresh activity on a low-ranked post (one recording CTE).
 *   4. Recompute just that post (one per-post CTE) and show /rising change.
...
async function main(): Promise<void> {
  ...
  console.log(`DEMO_NOW = ${DEMO_NOW.toISOString()}`);
  ...
  const db = await openDb();
  try {
    // ---- 2. Recompute (bulk CTE) + the two timelines -----------------
    await recomputeAllRisingScores(db, { now: DEMO_NOW });
    ...
    // ---- 3. Record fresh activity on the target (recording CTE) ------
    ...
    // ---- 4. Recompute just that post (per-post CTE) ------------------
    const recomputed = await recomputePostRisingScore(db, {
      postId: target.id,
      now: DEMO_NOW,
    });
    console.log(
    ...
    await recomputeAllRisingScores(db, { now: later });
    ...
    console.log("\n✓ Neon rising-feed (CTE) demo complete.");
  } finally {
    await db.close();
  }
}
