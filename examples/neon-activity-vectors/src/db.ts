/**
 * Connection helpers for the Neon activity-vectors example.
 *
 * Two connection roles, deliberately separated (mirrors the other Neon
 * examples):
 *
 * - {@link openDb} uses `DATABASE_URL` — the app/runtime path. Prefer the pooled
 *   Neon URL here; every runtime query in this example is a single
 *   self-contained statement, which is what Neon serverless / Deno Deploy is
 *   good at.
