 ...
 * aggregates).
 *
 * Timestamps use `mode: "date"` (JS `Date`), the explicit legacy mode — this
 * example keeps the simple Date model rather than Sisal's default Temporal
 * types (the Neon sibling uses Temporal; both are valid). `double precision`
 * columns infer `number`.
 *
...
export const posts = defineTable("posts", {
  ...
  // `.optional()`/`.default()`), so seeds pass an explicit `null`.
