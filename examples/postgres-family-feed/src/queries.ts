 ...
 *
 * `@sisal/pg` returns `double precision` columns as strings today (integers come
 * back as numbers; see the README and v0.5.0 roadmap item 11), so `rising_score`
 * is `Number(...)`-coerced at the boundary — in the feed rows and in the
 * `/rising` cursor — to honor `FeedPost.rising_score`'s typed `number`.
 *
 * @module
 */
...
// Coerce double precision (returned as a string by @sisal/pg) to a real number.
function toFeedPost(row: FeedPost): FeedPost {
...
export async function getNewFeed(
  ...
  return {
    posts: page.rows.map(toFeedPost),
...
export async function getRisingFeed(
  ...
  const nextCursor = next === undefined
    ? undefined
    : { ...next, rising_score: Number(next.rising_score) };
  return { posts: page.rows.map(toFeedPost), nextCursor };
}
