 ...
 *
 * Both use keyset pagination. The MySQL-family scenario suite pins both the
 * expanded and row-value forms live; this example uses the row-value form for
 * the multi-column `/rising` cursor.
 *
 * @module
 */
...
function normalize(rows: readonly FeedPost[]): FeedPost[] {
  return rows.map((row) => ({
    ...row,
