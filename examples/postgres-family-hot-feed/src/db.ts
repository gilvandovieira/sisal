/**
 * Connection helpers for the PostgreSQL-**family** hot-feed example.
 *
 ...
 * every other module runs unchanged. The serverless / Deno Deploy story still
 * holds when `SISAL_ADAPTER=neon`: every runtime query here is a single,
 * self-contained statement.
 *
 * - {@link openDb} uses `DATABASE_URL` — the app/runtime path (a pooled Neon URL
 *   is a good fit under `neon`).
